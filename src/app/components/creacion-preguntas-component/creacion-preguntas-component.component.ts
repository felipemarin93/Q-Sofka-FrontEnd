...
export class CreacionPreguntasComponentComponent implements OnInit {
  ...
  opcionesTipoPregunta: string[] = [
    "Opción múltiple", "Única opción", "Verdadero o falso"
  ];
  opcionesAreaConocimiento: string[] = [
    "Client-side", "Server-side", "Buenas prácticas", "Metodología"
  ];
  opcionesDescriptores: string[] = [
  ...
  persistirOpcion(namekey: string, value: string) {
    this.cookieService.set(namekey, value, this.obtenerLimiteCookie(new Date()))
  }
